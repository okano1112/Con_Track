// WorkerStatsScreen.js
// ============================================================
// หน้าสถิติและประเมินช่าง + Normal Curve
// ============================================================
...
import { getWorkersWithRecords, createWorker, createWorkerRecord, deleteWorker } from './db';
import {
  IndividualSkillCurve, CollectivePerformanceCurve, MiniSkillIndicator
} from './NormalCurveComponents';

const ROLES = ['ช่างไม้', 'ช่างก่อ', 'ช่างฉาบ', 'ช่างเหล็ก/ผูกเหล็ก', 'ช่างปูน/เทปูน',
  'ช่างไฟฟ้า', 'ช่างประปา', 'ช่างแอร์', 'ช่างฝ้าเพดาน', 'ช่างกระเบื้อง',
  'ช่างทาสี', 'ช่างเชื่อม', 'คนงานทั่วไป', 'อื่นๆ'];
const GENDERS = ['ชาย', 'หญิง'];
const EMPLOYMENT_STATUSES = ['พนักงานประจำ', 'พนักงานรายวัน', 'ผู้รับเหมาช่วง'];
const OT_HOURS_OPTIONS = ['0', '1', '1.5', '2', '2.5', '3', '4', '5', '6', '7', '8'];

// ============================================================
// WORK_TYPES (อ้างอิง สพฐ. + กรมบัญชีกลาง)
// ============================================================
const WORK_TYPES = [
  { key: 'โครงสร้าง คสล.', icon: 'business-outline', color: '#3B82F6', ref: 'สพฐ.: 2.4-3.2 ลบ.ม./คน/วัน', standard: 2.8, stdDev: 0.6, unit: 'ลบ.ม./วัน' },
  { key: 'ผูกเหล็ก', icon: 'construct-outline', color: '#2563EB', ref: 'สพฐ.: 150-200 กก./คน/วัน', standard: 175, stdDev: 35, unit: 'กก./วัน' },
  { key: 'เทปูน', icon: 'cube-outline', color: '#8B5CF6', ref: 'สพฐ.: 3-5 ลบ.ม./คน/วัน', standard: 4, stdDev: 0.8, unit: 'ลบ.ม./วัน' },
  { key: 'ตั้งแบบ', icon: 'copy-outline', color: '#1D4ED8', ref: 'สพฐ.: 4-6 ตร.ม./คน/วัน', standard: 5, stdDev: 1, unit: 'ตร.ม./วัน' },
  { key: 'ก่ออิฐมอญ', icon: 'grid-outline', color: '#F59E0B', ref: 'สพฐ.: 6-8 ตร.ม./คน/วัน', standard: 7, stdDev: 1.5, unit: 'ตร.ม./วัน' },
  { key: 'ก่ออิฐบล็อก', icon: 'grid-outline', color: '#D97706', ref: 'สพฐ.: 8-12 ตร.ม./คน/วัน', standard: 10, stdDev: 2, unit: 'ตร.ม./วัน' },
  { key: 'ก่ออิฐมวลเบา', icon: 'grid-outline', color: '#B45309', ref: 'สพฐ.: 10-15 ตร.ม./คน/วัน', standard: 12, stdDev: 2.5, unit: 'ตร.ม./วัน' },
  { key: 'ฉาบปูน', icon: 'layers-outline', color: '#10B981', ref: 'สพฐ.: 8-12 ตร.ม./คน/วัน', standard: 10, stdDev: 2, unit: 'ตร.ม./วัน' },
  { key: 'งานไม้แบบ', icon: 'hammer-outline', color: '#EC4899', ref: 'สพฐ.: 4-6 ตร.ม./คน/วัน', standard: 5, stdDev: 1, unit: 'ตร.ม./วัน' },
  { key: 'งานไม้คร่าว/โครง', icon: 'hammer-outline', color: '#DB2777', ref: 'สพฐ.: 10-15 ตร.ม./คน/วัน', standard: 12, stdDev: 2.5, unit: 'ตร.ม./วัน' },
  { key: 'งานไฟฟ้า', icon: 'flash-outline', color: '#F97316', ref: 'สพฐ.: 8-12 จุด/คน/วัน', standard: 10, stdDev: 2, unit: 'จุด/วัน' },
  { key: 'งานประปา', icon: 'water-outline', color: '#06B6D4', ref: 'สพฐ.: 6-10 จุด/คน/วัน', standard: 8, stdDev: 1.5, unit: 'จุด/วัน' },
  { key: 'งานทาสี', icon: 'color-palette-outline', color: '#10B981', ref: 'สพฐ.: 35-50 ตร.ม./คน/วัน', standard: 40, stdDev: 8, unit: 'ตร.ม./วัน' },
  { key: 'งานกระเบื้องพื้น', icon: 'apps-outline', color: '#EC4899', ref: 'สพฐ.: 6-10 ตร.ม./คน/วัน', standard: 8, stdDev: 1.5, unit: 'ตร.ม./วัน' },
  { key: 'งานกระเบื้องผนัง', icon: 'apps-outline', color: '#BE185D', ref: 'สพฐ.: 4-7 ตร.ม./คน/วัน', standard: 5.5, stdDev: 1, unit: 'ตร.ม./วัน' },
  { key: 'งานฝ้าเพดาน', icon: 'resize-outline', color: '#A855F7', ref: 'สพฐ.: 10-15 ตร.ม./คน/วัน', standard: 12, stdDev: 2.5, unit: 'ตร.ม./วัน' },
  { key: 'งานเชื่อม', icon: 'flame-outline', color: '#EF4444', ref: 'สพฐ.: 15-25 เมตร/คน/วัน', standard: 20, stdDev: 4, unit: 'ม./วัน' },
  { key: 'มุงหลังคาเมทัลชีท', icon: 'home-outline', color: '#6366F1', ref: 'สพฐ.: 20-30 ตร.ม./คน/วัน', standard: 25, stdDev: 5, unit: 'ตร.ม./วัน' },
  { key: 'มุงหลังคากระเบื้อง', icon: 'home-outline', color: '#4F46E5', ref: 'สพฐ.: 10-15 ตร.ม./คน/วัน', standard: 12, stdDev: 2.5, unit: 'ตร.ม./วัน' },
  { key: 'อื่นๆ', icon: 'ellipsis-horizontal-outline', color: '#6B7280', ref: 'กำหนดเอง', standard: 10, stdDev: 2, unit: 'หน่วย/วัน' },
];
...
function getMainWorkType(records) {
  ...
  const counts = {};
  records.forEach(r => { counts[r.work_type] = (counts[r.work_type] || 0) + 1; });
  ...
  <View style={{ flexDirection: 'row', marginVertical: 10, alignItems: 'center' }}>
    {[1, 2, 3, 4, 5].map((star) => (
      <TouchableOpacity key={star} onPress={() => onRatingChange(star)}>
        <Ionicons name={rating >= star ? 'star' : 'star-outline'} size={36}
          color={rating >= star ? '#F59E0B' : '#D1D5DB'} style={{ marginRight: 8 }} />
      </TouchableOpacity>
    ))}
...
export default function WorkerStatsScreen({ navigation }) {
  ...
  const [recordForm, setRecordForm] = useState({
    workType: 'ผูกเหล็ก', date: new Date(), output: '', qualityStar: 5, otHours: '0'
  });
  const [saving, setSaving] = useState(false);

  const loadData = async () => {
    try { setWorkers(await getWorkersWithRecords()); }
    catch (e) { console.log('Load workers error:', e); }
  };
  useFocusEffect(useCallback(() => { loadData(); }, []));

  const getTeamCurveData = (workType) => {
    const wt = WORK_TYPES.find(w => w.key === workType) || WORK_TYPES[0];
    const workersWithData = workers.map(w => {
      const avgOutput = calcAvgByWorkType(w.records, workType, 'output');
      if (avgOutput > 0) return { id: w.id, name: w.name, output: avgOutput };
      return null;
    }).filter(w => w !== null);
    return { workersWithData, mean: wt.standard, stdDev: wt.stdDev, unit: wt.unit };
  };
  ...
  const handleAddWorker = async () => {
    if (!workerForm.name.trim()) return Alert.alert('แจ้งเตือน', 'กรุณากรอกชื่อ');
    if (!workerForm.roles.length) return Alert.alert('แจ้งเตือน', 'กรุณาเลือกตำแหน่ง');
    if (!workerForm.nationality.trim()) return Alert.alert('แจ้งเตือน', 'กรุณาระบุสัญชาติ');
    ...
    try {
      await createWorker({
        ...
        avatarUrl: workerForm.avatarUri,
      });
      setWorkerForm({
        name: '', roles: [], customRole: '', age: '', phone: '', avatarUri: '',
        nationality: '', gender: 'ชาย', dailyWage: '', experienceYears: '',
  ...
  const handleAddRecord = async () => {
    ...
    const qualityPercent = recordForm.qualityStar * 20;
    if (isNaN(output) || output < 0) return Alert.alert('แจ้งเตือน', 'ตรวจสอบตัวเลขผลผลิต');
  ...
  const handleDelete = (w) => {
    Alert.alert('ลบข้อมูล', `ลบประวัติของ "${w.name}"?`, [
      { text: 'ยกเลิก' },
      {
  ...
  const Avatar = ({ worker, size = 48 }) => {
    if (worker.avatar_url) {
      return <Image source={{ uri: worker.avatar_url }}
  ...
  const ranked = [...workers].map(w => ({ ...w, score: getScore(w) }))
          ...
          <ScrollView style={{ maxHeight: 300, width: '100%', marginVertical: 10 }}>
            {pickerConfig.options.map(opt => {
              const isSelected = pickerConfig.field === 'otHours'
                ...
                : pickerConfig.isMulti
                ? (workerForm[pickerConfig.field] || []).includes(opt)
                : workerForm[pickerConfig.field] === opt;
              return (
                <TouchableOpacity key={opt} onPress={() => handleSelectPickerOption(opt)}
                  style={s.pickerItem}>
                  <Text style={{
                    fontSize: 16, color: isSelected ? C.primary : C.text,
      ...
      <View style={s.overlay}>
        <View style={[s.modal, { maxHeight: '90%' }]}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <Input label="ชื่อ-นามสกุล *" value={workerForm.name}
              onChangeText={v => setWorkerForm(p => ({ ...p, name: v }))}
              placeholder="เช่น สมชาย ใจดี" icon="person-outline" />

            <Text style={s.label}>ตำแหน่งงาน (เลือกได้มากกว่า 1) *</Text>
            <TouchableOpacity style={s.dropdownBtn}
              onPress={() => openPicker('เลือกตำแหน่งงาน', ROLES, 'roles', true)}>
              <Text style={s.dropdownTxt}>
                {workerForm.roles.length > 0 ? workerForm.roles.join(', ') : 'กดเพื่อเลือก'}
              </Text>
              <Ionicons name="chevron-down" size={18} color={C.textSec} />
            </TouchableOpacity>
            {workerForm.roles.includes('อื่นๆ') && (
              <Input label="ระบุตำแหน่ง" value={workerForm.customRole}
                onChangeText={v => setWorkerForm(p => ({ ...p, customRole: v }))}
                placeholder="กรอกตำแหน่ง" icon="create-outline" />
            )}

            <Input label="สัญชาติ *" value={workerForm.nationality}
              onChangeText={v => setWorkerForm(p => ({ ...p, nationality: v }))}
              placeholder="เช่น ไทย, เมียนมา" icon="flag-outline" />

            <View style={{ flexDirection: 'row', gap: 10 }}>
              <View style={{ flex: 1 }}>
                <Text style={s.label}>เพศ</Text>
                <TouchableOpacity style={s.dropdownBtn}
                  onPress={() => openPicker('เลือกเพศ', GENDERS, 'gender')}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <TouchableOpacity style={s.dropdownBtn}
                  onPress={() => openPicker('สถานะการจ้าง', EMPLOYMENT_STATUSES, 'employmentStatus')}>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <View style={{ flex: 1 }}>
                <Input label="อายุ (ปี)" value={workerForm.age}
                  onChangeText={v => setWorkerForm(p => ({ ...p, age: v }))}
                  keyboardType="numeric" icon="calendar-outline" />
              </View>
              <View style={{ flex: 1 }}>
                <Input label="อายุงาน (ปี)" value={workerForm.experienceYears}
                  onChangeText={v => setWorkerForm(p => ({ ...p, experienceYears: v }))}
                  keyboardType="numeric" icon="briefcase-outline" />
              </View>
            </View>

            <View style={{ flexDirection: 'row', gap: 10 }}>
              <View style={{ flex: 1 }}>
                <Input label="ค่าแรง (บ./วัน)" value={workerForm.dailyWage}
                  onChangeText={v => setWorkerForm(p => ({ ...p, dailyWage: v }))}
                  keyboardType="numeric" icon="cash-outline" />
              </View>
              <View style={{ flex: 1 }}>
                <Input label="เบอร์โทร" value={workerForm.phone}
                  onChangeText={v => setWorkerForm(p => ({ ...p, phone: v }))}
                  keyboardType="phone-pad" icon="call-outline" />
              </View>
            </View>

            <Button title="บันทึกข้อมูล" onPress={handleAddWorker} loading={saving}
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {WORK_TYPES.map(wt => {
                  ...
                  return (
                    <TouchableOpacity key={wt.key}
                      onPress={() => setRecordForm(p => ({ ...p, workType: wt.key }))}
                      style={{
                        paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20,
                        backgroundColor: sel ? wt.color : '#F3F4F6',
                        borderWidth: sel ? 0 : 1, borderColor: C.border
                      }}>
                      <Text style={{ color: sel ? '#fff' : C.textSec, fontWeight: '600', fontSize: 12 }}>
                        {wt.key}
                      </Text>
                    </TouchableOpacity>
                  );
              ...
              <TouchableOpacity onPress={() => setShowDatePicker(true)} style={s.dateBtn}>
                ...
                <Text style={{ fontSize: 16, color: C.text }}>
                  {recordForm.date.toLocaleDateString('th-TH')}
                </Text>
              </TouchableOpacity>

              {showDatePicker && (
                <DateTimePicker value={recordForm.date} mode="date"
              ...
              <Input label={`ปริมาณผลผลิต (${activeWork.unit}) *`}
                value={recordForm.output}
                onChangeText={v => setRecordForm(p => ({ ...p, output: v }))}
                placeholder="เช่น 20" keyboardType="numeric" icon="trending-up-outline" />

              <Text style={s.label}>ประเมินคุณภาพ (ดาว) *</Text>
              <StarRating rating={recordForm.qualityStar}
                onRatingChange={(stars) => setRecordForm(p => ({ ...p, qualityStar: stars }))} />

              <Text style={s.label}>ชั่วโมง OT</Text>
              <TouchableOpacity style={s.dropdownBtn}
                onPress={() => openPicker('เลือกชั่วโมง OT', OT_HOURS_OPTIONS, 'otHours')}>
              ...
              {workerWage > 0 ? (
                <View style={{
                  backgroundColor: '#FFF7ED', padding: 14, borderRadius: 10, marginBottom: 20,
                  borderWidth: 1, borderColor: '#FED7AA'
                }}>
                  <Text style={{ fontSize: 12, color: '#C2410C', marginBottom: 6 }}>
                    ฐานค่าแรง: {workerWage} บ./วัน
                  </Text>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                    <Text style={{ fontSize: 14, color: '#EA580C', fontWeight: '600' }}>ค่า OT สุทธิ</Text>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={{ fontSize: 15, color: '#9A3412', fontWeight: '700' }}>รวมทั้งหมด</Text>
                    <Text style={{ fontSize: 20, color: '#9A3412', fontWeight: '900' }}>{totalPay.toFixed(2)} ฿</Text>
                  </View>
                </View>
              ) : (
                <Text style={{ fontSize: 12, color: '#EF4444', marginBottom: 20 }}>
                  * ไม่มีข้อมูลค่าแรง คำนวณ OT ไม่ได้
                </Text>
              )}

              <Button title="บันทึกสถิติ" onPress={handleAddRecord} loading={saving}
  ...
  const renderDetailModal = () => {
    ...
    const mainAvg = mainWorkType
      ? calcAvgByWorkType(selectedWorker.records, mainWorkType, 'output') : 0;
          ...
          <View style={[s.modal, { maxHeight: '90%' }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={{ alignItems: 'center', marginBottom: 20 }}>
                ...
                {selectedWorker.experience_years > 0 && (
                  <Text style={{ fontSize: 12, color: C.textSec, marginTop: 2 }}>
                    ประสบการณ์: {selectedWorker.experience_years} ปี
                  </Text>
                )}
                {selectedWorker.daily_wage > 0 && (
                  <Text style={{ fontSize: 13, color: C.primary, marginTop: 4, fontWeight: 'bold' }}>
                    ค่าแรง: {selectedWorker.daily_wage} บ./วัน
                  </Text>
                )}
                <Badge label={`เกรด ${grade.label} • ${score}%`} color={grade.color} bg={grade.bg} />
              </View>

              {mainWorkTypeData && mainAvg > 0 && (
                <IndividualSkillCurve
                  currentWorkerOutput={mainAvg}
              ...
              <Card>
                ...
                {selectedWorker.records?.length > 0 ? (
                  selectedWorker.records.slice(0, 5).map(r => (
                    <Text key={r.id} style={{ fontSize: 13, color: C.textSec, lineHeight: 22 }}>
                      • {r.date} | {r.work_type} (ผลผลิต: {r.output}, OT: {r.ot_hours || 0} ชม.)
                    </Text>
                  ))
                ) : (
              ...
              <Button title="ลบข้อมูล" variant="danger" icon="trash-outline"
                onPress={() => handleDelete(selectedWorker)}
    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
            ...
            return (
              <TouchableOpacity key={t.key} onPress={() => setActiveTab(t.key)}
                style={{
                  ...
                  backgroundColor: act ? '#fff' : 'rgba(255,255,255,0.1)'
                }}>
                <Ionicons name={t.icon} size={16} color={act ? C.primary : '#fff'} />
                <Text style={{ fontSize: 14, fontWeight: '600', color: act ? C.primary : '#fff' }}>
                  {t.label}
                </Text>
              </TouchableOpacity>
            );
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 30 }}
        ...
        {activeTab === 'individual' ? (
          <View>
            <Button title="เพิ่มพนักงานใหม่" icon="person-add-outline"
              onPress={() => setShowAddWorker(true)} style={{ marginBottom: 16 }} />
            {workers.length > 0 ? workers.map(w => {
              ...
              const mainAvg = mainWorkType ? calcAvgByWorkType(w.records, mainWorkType, 'output') : 0;
                ...
                <Card key={w.id} onPress={() => { setSelectedWorker(w); setShowDetail(true); }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                      ...
                      {mainWorkTypeData && mainAvg > 0 && (
                        <MiniSkillIndicator output={mainAvg}
                          mean={mainWorkTypeData.standard}
                          stdDev={mainWorkTypeData.stdDev} width={70} />
                      )}
                    </View>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }}>
                    <Text style={{ fontSize: 11, color: C.textLight }}>
                      สถิติ {w.records?.length || 0} รายการ
                    </Text>
                    <TouchableOpacity onPress={() => {
                      ...
                      <Text style={{ fontSize: 11, color: C.primary, fontWeight: '600', marginLeft: 4 }}>
                        เพิ่มสถิติ
                      </Text>
                    </TouchableOpacity>
                  </View>
              ...
              <Empty icon="people-outline" title="ยังไม่มีข้อมูลพนักงาน"
                subtitle="กดเพิ่มพนักงานใหม่ด้านบน" />
            )}
          </View>
        ) : (
          <View>
            <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
              {[
                ...
                { val: workers.reduce((s, w) => s + (w.records?.length || 0), 0), label: 'สถิติทั้งหมด', color: '#10B981', bg: '#D1FAE5' },
                { val: ranked[0] ? `${ranked[0].score}%` : '-', label: 'สูงสุด', color: '#F59E0B', bg: '#FEF3C7' },
              ].map((d, i) => (
                <View key={i} style={{
                  flex: 1, backgroundColor: d.bg, borderRadius: 14,
            ...
            <Card style={{ marginBottom: 16 }}>
              ...
              <Text style={{ fontSize: 12, color: C.textSec, marginBottom: 8 }}>เลือกประเภทงาน</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  {WORK_TYPES.filter(w => w.key !== 'อื่นๆ').map(wt => {
                    const isSel = dashboardWorkType === wt.key;
                    return (
                      <TouchableOpacity key={wt.key} onPress={() => setDashboardWorkType(wt.key)}
                        style={{
                          flexDirection: 'row', alignItems: 'center', gap: 6,
                          paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20,
                          backgroundColor: isSel ? wt.color : '#F3F4F6',
                          borderWidth: isSel ? 0 : 1, borderColor: C.border,
                        }}>
                        <Ionicons name={wt.icon} size={14} color={isSel ? '#fff' : C.textSec} />
                        <Text style={{ color: isSel ? '#fff' : C.textSec, fontWeight: '600', fontSize: 11 }}>
                          {wt.key}
                        </Text>
                      </TouchableOpacity>
                    );
            ...
            {(() => {
              ...
              if (workersWithData.length === 0) {
                return (
                  <Card>
                    <Empty icon="analytics-outline"
                      title={`ยังไม่มีข้อมูลงาน "${dashboardWorkType}"`}
                      subtitle="เพิ่มสถิติเพื่อดูกราฟ" />
                  </Card>
                );
              }
              return (
                <CollectivePerformanceCurve workersData={workersWithData}
                  workTypeMean={mean} workTypeSD={stdDev}
                  workTypeName={dashboardWorkType} unit={unit}
                  onWorkerPress={(w) => {
                    const full = workers.find(fw => fw.id === w.id);
                    if (full) { setSelectedWorker(full); setShowDetail(true); }
                  }} />
              );
            })()}

            <Text style={{ fontSize: 17, fontWeight: '700', color: C.text, marginTop: 16, marginBottom: 12 }}>
              จัดอันดับช่าง
            </Text>
                ...
                <Card key={w.id} onPress={() => { setSelectedWorker(w); setShowDetail(true); }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <View style={{
                      ...
                      backgroundColor: i < 3 ? grade.bg : '#F3F4F6',
                      alignItems: 'center', justifyContent: 'center'
                    }}>
                      <Text style={{
                        fontSize: i < 3 ? 18 : 14, fontWeight: '700',
                        color: i < 3 ? grade.color : C.textSec
                      }}>{medal}</Text>
                    </View>
                  ...
                  <ProgressBar progress={w.score} height={5} color={grade.color}
                    style={{ marginTop: 10 }} />
                </Card>
              );
