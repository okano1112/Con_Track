  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.bg }}
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }} keyboardShouldPersistTaps="handled">
        <Input label="ชื่อโครงการ *" value={form.name} onChangeText={v => u('name', v)}
        ...
        <Input label="สถานที่" value={form.location} onChangeText={v => u('location', v)}
          placeholder="เช่น กรุงเทพฯ" icon="location-outline" />
        <Input label="งบประมาณ (บาท)" value={form.budget} onChangeText={v => u('budget', v)}
          placeholder="0" keyboardType="numeric" icon="cash-outline" />
        <Input label="วันเริ่ม" value={form.startDate} onChangeText={v => u('startDate', v)}
          placeholder="YYYY-MM-DD" icon="calendar-outline" />
        <Input label="วันสิ้นสุด" value={form.endDate} onChangeText={v => u('endDate', v)}
          placeholder="YYYY-MM-DD" icon="calendar-outline" />
...
export function ProjectDetailScreen({ route, navigation }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      <Header title={project.name} subtitle={st.label}
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}
        ...
        {tab === 'info' && (
          <Card>
            ...
            {project.description ? (
              <Text style={{ color: C.textSec, marginTop: 8, lineHeight: 22 }}>
                {project.description}
              </Text>
            ) : null}
          </Card>
