...
export default function DocumentsScreen() {
  ...
  const load = async () => {
    try { setDocs(await getAllDocuments(filter)); } catch (e) { console.log(e); }
  };
  useFocusEffect(useCallback(() => { load(); }, [filter]));

  const FILTERS = [{ key: 'all', label: 'ทั้งหมด' }, ...Object.entries(DOC_CAT).map(([k, v]) => ({ key: k, label: v.label }))];
            ...
            <Card key={doc.id}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ fontSize: 12, color: C.textSec }}>{doc.project_name || '-'} • {cat.label}</Text>
                </View>
                <TouchableOpacity onPress={() => Alert.alert('ลบ', `ลบ "${doc.name}"?`, [
                  { text: 'ยกเลิก' }, { text: 'ลบ', style: 'destructive', onPress: async () => { await deleteDocument(doc.id); load(); } }
